...
test('A route could support multiple versions (find) / 3', (t) => {
  ...
  findMyWay.on(
    'GET',
    '/test/:id/hello',
    { constraints: { version: '3.2.0' } },
    noop
  )
  findMyWay.on(
    'GET',
    '/test/name/hello',
    { constraints: { version: '4.0.0' } },
    noop
  )

  t.assert.ok(findMyWay.find('GET', '/test/1234/hello', { version: '1.x' }))
  t.assert.ok(
    findMyWay.find('GET', '/test/1234/hello', { version: '1.2.3' })
  )
  t.assert.ok(findMyWay.find('GET', '/test/1234/hello', { version: '3.x' }))
  t.assert.ok(
    findMyWay.find('GET', '/test/1234/hello', { version: '3.2.0' })
  )
  ...
  t.assert.ok(
    !findMyWay.find('GET', '/test/1234/hello', { version: '2.3.4' })
  )
  t.assert.ok(
    !findMyWay.find('GET', '/test/1234/hello', { version: '3.2.1' })
  )
...
test('It should throw if you declare multiple times the same route', (t) => {
  ...
  try {
    ...
    t.assert.equal(
      err.message,
      "Method 'GET' already declared for route '/' with constraints '{\"version\":\"1.2.3\"}'"
    )
  }
})
