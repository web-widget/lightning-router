...
test('should setup parametric and multi-parametric node', (t) => {
  ...
  t.assert.equal(findMyWay.find('GET', '/foo/value').handler, paramHandler)
  t.assert.equal(
    findMyWay.find('GET', '/foo/value.png').handler,
    regexpHandler
  )
})
...
test('should set up two parametric nodes with static ending', (t) => {
  ...
  t.assert.equal(
    findMyWay.find('GET', '/foo/value.jpeg').handler,
    paramHandler2
  )
})
...
test('should set up two regexp nodes with static ending', (t) => {
  ...
  t.assert.equal(
    findMyWay.find('GET', '/foo/123.png').handler,
    paramHandler1
  )
  t.assert.equal(
    findMyWay.find('GET', '/foo/456.jpeg').handler,
    paramHandler2
  )
})
...
test('node with longer static suffix should have higher priority', (t) => {
  ...
  t.assert.equal(
    findMyWay.find('GET', '/foo/value.png.png').handler,
    paramHandler2
  )
})
...
test('should set up regexp node and node with static ending', (t) => {
  ...
  t.assert.equal(
    findMyWay.find('GET', '/foo/123.jpeg').handler,
    multiParamHandler
  )
