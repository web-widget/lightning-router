...
test('A route could support a custom constraint strategy', (t) => {
  ...
  t.assert.equal(
    findMyWay.find('GET', '/', { requestedBy: 'curl' }).handler,
    alpha
  )
  t.assert.equal(
    findMyWay.find('GET', '/', { requestedBy: 'wget' }).handler,
    beta
  )
...
test('A route could support a custom constraint strategy while versioned', (t) => {
  ...
  t.assert.equal(
    findMyWay.find('GET', '/', { requestedBy: 'curl', version: '1.x' })
      .handler,
    alpha
  )
  t.assert.equal(
    findMyWay.find('GET', '/', { requestedBy: 'curl', version: '2.x' })
      .handler,
    beta
  )
  t.assert.equal(
    findMyWay.find('GET', '/', { requestedBy: 'wget', version: '2.x' })
      .handler,
    gamma
  )
  t.assert.equal(
    findMyWay.find('GET', '/', { requestedBy: 'wget', version: '3.x' })
      .handler,
    delta
  )
  ...
  t.assert.ok(
    !findMyWay.find('GET', '/', { requestedBy: 'curl', version: '3.x' })
  )
  t.assert.ok(
    !findMyWay.find('GET', '/', { requestedBy: 'wget', version: '1.x' })
  )
})
...
test('Custom constraint strategies can set mustMatchWhenDerived flag to true which prevents matches to unconstrained routes when a constraint is derived and there are no other routes', (t) => {
  ...
  findMyWay.lookup(
    { method: 'GET', url: '/', headers: { 'user-agent': 'node' } },
    null
  )
})
...
test('Custom constraint strategies can set mustMatchWhenDerived flag to true which prevents matches to unconstrained routes when a constraint is derived when there are constrained routes', (t) => {
  ...
  findMyWay.on('GET', '/', {}, () => t.assert.assert.fail())
  findMyWay.on('GET', '/', { constraints: { requestedBy: 'curl' } }, () =>
    t.assert.assert.fail()
  )
  findMyWay.on('GET', '/', { constraints: { requestedBy: 'wget' } }, () =>
    t.assert.assert.fail()
  )
