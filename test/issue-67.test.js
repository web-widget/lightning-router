...
test('parametric routes', (t) => {
  ...
  t.assert.equal(
    findMyWay.find('GET', '/foo/awesome-parameter').handler,
    foo
  )
  t.assert.equal(
    findMyWay.find('GET', '/foo/b-first-character').handler,
    foo
  )
  t.assert.equal(
    findMyWay.find('GET', '/foo/s-first-character').handler,
    foo
  )
...
test('parametric with common prefix', (t) => {
  ...
  findMyWay.on('GET', '/:test', (req, res, params) => {
    t.assert.deepEqual({ test: 'text' }, params)
  })
