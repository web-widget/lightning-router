import acceptVersionStrategy from './strategies/accept-version.js'
import acceptHostStrategy from './strategies/accept-host.js'
...
class Constrainer {
  ...
  hasConstraintStrategy (strategyName) {
    ...
    if (customConstraintStrategy !== undefined) {
      return (
        customConstraintStrategy.isCustom ||
  ...
  validateConstraints (constraints) {
    for (const key in constraints) {
      ...
      if (!strategy) {
        throw new Error(
                    `No strategy registered for constraint key ${key}`
        )
      }
  ...
  _buildDeriveConstraints () {
    ...
    this.deriveSyncConstraints = new Function( // eslint-disable-line no-new-func
      'req',
      'ctx',
      lines.join('\n')
    ).bind(this)
